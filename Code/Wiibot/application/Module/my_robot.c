...
void My_Model_Output_Cal(void)
{
  K_Matrix_t *K = &My_K_Matrix;
	...
	/* 左驱动轮扭矩输出计算 */
	My_Straight_Leg_Model.l_s_part = K->wheell_K[0] * My_State_Var.s + K->wheell_K[1] * My_State_Var.sd1;
	
	My_Straight_Leg_Model.l_theta_part = K->wheell_K[2] * My_State_Var.thetab + K->wheell_K[3] * My_State_Var.thetabd1;
...
//	My_Straight_Leg_Model.l_phi_part = K->wheell_K[4] * My_Yaw_Zero_Point_Process(My_State_Var.phi) + K->wheell_K[5] * My_State_Var.phid1;
  ...
  My_Straight_Leg_Model.L_Tw = -( My_Straight_Leg_Model.l_s_part + My_Straight_Leg_Model.l_theta_part + My_Straight_Leg_Model.l_phi_part );
	...
	/* 右驱动轮扭矩输出计算 */
	My_Straight_Leg_Model.r_s_part = K->wheelr_K[0] * My_State_Var.s + K->wheelr_K[1] * My_State_Var.sd1;
	
	My_Straight_Leg_Model.r_theta_part = K->wheelr_K[2] * My_State_Var.thetab + K->wheelr_K[3] * My_State_Var.thetabd1;
